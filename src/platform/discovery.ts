 ...
 *
 * Strategy:
 * 1. Find ALL LS processes via `ps` (Linux/macOS) or `wmic` (Windows) → extract --csrf_token and --workspace_id
...
export async function discoverLanguageServer(host: string): Promise<ServerConnection | null> {
  ...
  try {
    ...
    // Normalize wmic CSV output to match ps-style "pid ppid args" format
    const lines: string[] = [];
